...
class LocationService {
    ...
    fun voivodeshipList(): List<Pair<String, String>> {
        val ids = compileAndEvaluateXPath(
            """//row[NAZWA_DOD="województwo"]/WOJ/text()""", terc
        ).toList()

        return compileAndEvaluateXPath(
            """//row[NAZWA_DOD="województwo"]/NAZWA/text()""", terc
        ).toListOfPairs(ids)
    }
    ...
    fun countyList(voivodeshipId: String): List<Pair<String, String>> {
        val ids = compileAndEvaluateXPath(
            """//row[WOJ="$voivodeshipId" and 
                    ...
                    |]/POW/text()""".trimMargin(), terc
        ).toList()

        return compileAndEvaluateXPath(
            """//row[WOJ="$voivodeshipId" and 
                    |(NAZWA_DOD="powiat" or NAZWA_DOD="miasto na prawach powiatu")
                    |]/NAZWA/text()""".trimMargin(), terc
        ).toListOfPairs(ids)
    }

    fun communeList(
        voivodeshipId: String, countyId: String
    ...
    fun localityList(
        voivodeshipId: String, countyId: String, communeId: String
    ): List<Pair<String, String>> {
        val ids = compileAndEvaluateXPath(
